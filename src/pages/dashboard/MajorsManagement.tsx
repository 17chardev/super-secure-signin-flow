...
const MajorsManagement = () => {
  ...
  const fetchMajors = async () => {
    try {
      setLoading(true);
      const result = await fetchPaginated<Major>('majors', pagination);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast.error(error.message || 'Failed to save major');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error.message || 'Failed to delete major');
    }
  };
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">        
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <div className="max-w-7xl mx-auto">
            <DashboardHeader 
              title="Data Jurusan"
              subtitle="Kelola data jurusan di sekolah"
              ...
              <div className="overflow-x-auto">
                {loading ? (
                    ...
                    <TableBody>
                      {majors.map((major, index) => (
                        <TableRow key={major.id}>
                          ...
                          <TableCell>
                            {major.created_at 
                              ? format(new Date(major.created_at), 'dd/MM/yyyy HH:mm') 
                              : '-'}
                          </TableCell>
                  ...
                  <EmptyState
                    ...
                    description="Tambahkan jurusan baru untuk memulai"
                    actionLabel="Tambah Jurusan"
                    onAction={() => {
                      setIsEditing(false);
                      form.reset();
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{isEditing ? 'Edit Jurusan' : 'Tambah Jurusan Baru'}</DialogTitle>
            <DialogDescription>
              {isEditing 
                ? 'Edit detail jurusan yang sudah ada' 
                : 'Isi formulir berikut untuk menambahkan jurusan baru'}
            </DialogDescription>
          </DialogHeader>
